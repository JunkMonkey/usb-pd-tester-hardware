...
2. **USB-C 公头** 24 个引脚焊接是否饱满（SMD，插入被检测设备端），CC1/CC2 的 4.7kΩ 上拉电阻（到 3.3V）是否焊好
...
将 USB-C 母座端插充电器，USB-C 公头端通过数据线接负载，对比公头端电压 vs 充电器标称电压，可判断线材的压降和载流能力。
...
4. 🔌 **USB-C 公头输出端注意方向** — USB-C 支持正反插，但暴力插入可能损坏引脚
5. 💧 **不防水、不防尘** — 仅限室内桌面使用
